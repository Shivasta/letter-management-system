#!/usr/bin/env python3
"""
Letter Management System - Mobile Testing Runner
Run this to test the portal on your mobile device
Loads the legacy monolithic app.py directly to ensure all routes exist.
"""
...
def main():
    ...
    try:
        ...
        legacy_app = module_from_spec(spec)
        spec.loader.exec_module(legacy_app)

        app_obj = getattr(legacy_app, 'app', None)
        init_db = getattr(legacy_app, 'init_db', None)

        if app_obj is None or init_db is None:
            raise RuntimeError("Loaded module missing expected `app` or `init_db`.")
