...
app.config.update(
    ...
    # Use Lax to allow top-level navigations (works for IP access); keep non-secure for HTTP dev
...
# if not app.debug and os.environ.get('LMS_SECRET_KEY') in (None, '') and secret_from_env == os.urandom(32).hex():
#     raise RuntimeError("SECRET_KEY must be set via LMS_SECRET_KEY in production")
...
# Initialize rate limiter with in-memory storage explicitly (get_remote_address already imported above)
limiter = Limiter(
    get_remote_address,
    app=app,
    storage_uri="memory://",
...
logger = logging.getLogger('letter_management')
...
def create_tables_if_not_exist(cur, db):
    ...
    try:
        ...
        # Create notifications table (required by create_notification / mark_notification_as_read)
        cur.execute("""
...
def add_missing_columns(cur, db):
    ...
    try:
        ...
        if not cur.fetchone():
            ...
            """)
        # Check if status_changed_at column exists in requests table (used by mentor/advisor/hod_action)
        cur.execute("SHOW COLUMNS FROM requests LIKE 'status_changed_at'")
        if not cur.fetchone():
            cur.execute("""
                ALTER TABLE requests 
                ADD COLUMN status_changed_at TIMESTAMP NULL
            """)
        # Ensure notifications table has read_at if table was created without it
        cur.execute("SHOW TABLES LIKE 'notifications'")
        if cur.fetchone():
            cur.execute("SHOW COLUMNS FROM notifications LIKE 'read_at'")
            if not cur.fetchone():
                cur.execute("""
...
def create_notification(user_id, request_id, notification_type, title, message, status):
    """Create a notification for a user when request status changes"""
    try:
        db = get_db()
        if db is None:
            logger.warning("Could not create notification: DB connection failed")
            return False
        
        cur = db.cursor()
        cur.execute("""
            INSERT INTO notifications (user_id, request_id, type, title, message, status, is_read, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, FALSE, NOW())
        """, (user_id, request_id, notification_type, title, message, status))
        db.commit()
        cur.close()
...
def get_user_notifications(user_id, limit=10):
    ...
    try:
        ...
        cur.execute("""
            SELECT id, request_id, type, title, message, status, is_read, created_at
            FROM notifications
            WHERE user_id=%s
...
def mark_notification_as_read(notification_id, user_id):
    """Mark a notification as read; only succeeds if the notification belongs to user_id (prevents IDOR)."""
    try:
        db = get_db()
        if db is None:
            return False
...
DB_CONFIG = {
    ...
    'user': os.environ.get('LMS_DB_USER', 'root'),
    'password': os.environ.get('LMS_DB_PASSWORD', ''),
    'database': os.environ.get('LMS_DB_NAME', 'letter_management'),
    'autocommit': False
}
DB_ALTERNATIVES = [
    {'host': 'localhost', 'user': 'root', 'password': '', 'database': 'letter_management', 'autocommit': False},
    {'host': 'localhost', 'user': 'root', 'password': 'root', 'database': 'letter_management', 'autocommit': False}
]
USING_SQLITE = False
...
def get_db():
    ...
    if db is None:
        try:
            ...
            for alt_config in DB_ALTERNATIVES:
                try:
                    db = g._database = mysql.connector.connect(**alt_config)
                    return db
                except Error:
                    continue
...
def register():
    ...
    if request.method == 'POST':
        ...
        name = bleach.clean((request.form.get('name') or '').strip(), strip=True)
        ...
        if role == 'Mentor':
            ...
            register_number = bleach.clean((request.form.get('register_number') or '').strip(), strip=True)
        
        password = request.form.get('password', '')
        confirm = request.form.get('confirm_password', '')
        email = bleach.clean((request.form.get('email') or '').strip(), strip=True)
        ...
        dob = bleach.clean((request.form.get('dob') or '').strip(), strip=True)
        if not all([register_number, password, email, dob]):
            flash("Registration number, password, email, and date of birth are required", "danger")
            return render_template('register.html', mentors=mentors)
...
@csrf.exempt  # Relax CSRF for mobile testing and multi-device submissions
...
def get_notification_count():
    ...
    try:
        count = get_unread_notification_count(session['id'])
...
def get_notifications_api():
    ...
    try:
        ...
        limit = max(1, min(100, limit))  # Cap between 1 and 100 to avoid DoS
        notifications = get_user_notifications(session['id'], limit)
        return jsonify({'notifications': notifications, 'success': True})
    except Exception as e:
        logger.exception("Error getting notifications")
        return jsonify({'notifications': [], 'success': False, 'error': str(e)}), 500

@app.route('/api/notifications/<int:notification_id>/read', methods=['POST'])
@login_required
def mark_notification_read(notification_id):
    """API endpoint to mark notification as read (only for current user's notifications)."""
    try:
        success = mark_notification_as_read(notification_id, session['id'])
...
def download_status():
    ...
    try:
        ...
        try:
            ...
            pdf.cell(0, 10, txt=f"Generated by Letter Management System - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", ln=1, align='C')
...
def download_single_approved(req_id):
    ...
    try:
        ...
        # Add footer
        pdf.set_font("Arial", 'I', 10)
        pdf.cell(0, 10, txt="Generated by Letter Management System", ln=1, align='C')
...
def mentor_action():
    ...
    if not req_id or action not in ('Approve', 'Reject'):
        flash("Invalid request. Missing request_id or action.", "danger")
        return redirect(url_for('mentor'))
    try:
        req_id = int(req_id)
    ...
    try:
        ...
        # Get request user_id, type, department, status (ensure request exists and belongs to mentor's department)
        cur.execute("SELECT user_id, type, department, status FROM requests WHERE id=%s", (req_id,))
        result = cur.fetchone()
        if not result or result[3] != 'Pending':
            cur.close()
            flash("Request not found or not pending.", "danger")
            return redirect(url_for('mentor'))
        if normalize_department_name(result[2]) != normalize_department_name(session.get('department')):
            cur.close()
            flash("You can only act on requests from your department.", "danger")
            return redirect(url_for('mentor'))
        
        cur.execute("UPDATE requests SET status=%s, updated_at=NOW(), status_changed_at=NOW() WHERE id=%s", (status_value, req_id))
        db.commit()
        cur.close()
        
        # Create notification for the student
        if result:
            user_id, request_type = result[0], result[1]
            notification_title = f"{request_type} Request {action}ed by Mentor"
            notification_message = f"Your {request_type} request has been {action.lower()}ed by your mentor"
            create_notification(
                user_id, 
                int(req_id), 
                'approval',
        ...
        if 'cur' in locals():
            cur.close()
        logger.exception(f"Error in mentor_action: {e}")
...
def advisor_action():
    ...
    try:
        ...
        # Get request user_id, type, department, status (ensure request exists and is Mentor Approved in advisor's dept)
        cur.execute("SELECT user_id, type, department, status FROM requests WHERE id=%s", (req_id,))
        result = cur.fetchone()
        if not result or result[3] != 'Mentor Approved':
            cur.close()
            flash("Request not found or not mentor-approved.", "danger")
            return redirect(url_for('advisor'))
        if normalize_department_name(result[2]) != normalize_department_name(session.get('department')):
            cur.close()
            flash("You can only act on requests from your department.", "danger")
        ...
        if action == 'Approve':
            cur.execute("UPDATE requests SET status=%s, updated_at=NOW(), status_changed_at=NOW(), advisor_note=%s WHERE id=%s", (status_value, advisor_note, req_id))
        else:
            cur.execute("UPDATE requests SET status=%s, updated_at=NOW(), status_changed_at=NOW(), advisor_note=%s WHERE id=%s", (status_value, advisor_note, req_id))
        db.commit()
        cur.close()
        ...
        if result:
            user_id, request_type = result[0], result[1]
            notification_title = f"{request_type} Request {action}ed by Advisor"
            notification_message = f"Your {request_type} request has been {action.lower()}ed by your advisor"
            create_notification(
                user_id, 
                int(req_id), 
                'approval',
        ...
        if 'cur' in locals():
            cur.close()
        logger.exception(f"Error in advisor_action: {e}")
        flash("Error updating request status", "danger")
        return redirect(url_for('advisor'))
...
def hod_action():
    ...
    try:
        ...
        # Get request user_id, type, department (ensure request exists and belongs to HOD's department)
        cur.execute("SELECT user_id, type, department FROM requests WHERE id=%s", (req_id,))
        result = cur.fetchone()
        if not result:
            cur.close()
            flash("Request not found.", "danger")
        ...
        if result:
            user_id, request_type = result[0], result[1]
            notification_title = f"{request_type} Request {action}ed by HOD"
            notification_message = f"Your {request_type} request has been {action.lower()}ed by the HOD"
            create_notification(
                user_id, 
                int(req_id), 
                'approval',
        ...
        if 'cur' in locals():
            cur.close()
        logger.exception(f"Error in hod_action: {e}")
        flash("Error updating request status", "danger")
        return redirect(url_for('hod'))
...
def edit_user(user_id):
    ...
    try:
        ...
        if request.method == 'POST':
            # Update user details (use .get() to avoid KeyError on malformed POST)
            name = bleach.clean((request.form.get('name') or '').strip(), strip=True)
            email = bleach.clean((request.form.get('email') or '').strip(), strip=True)
            role = bleach.clean((request.form.get('role') or '').strip(), strip=True)
            department = bleach.clean((request.form.get('department') or '').strip(), strip=True)
